...
| `--color` | 일치 부분 색상 강조 |
| `--include` | 특정 확장자 파일만 검색 대상으로 지정 |
| `-e` | 검색 패턴 여러 개 지정 (expression) |
...
## 🗂️ 실전 사례 — 카카오 우편번호 마이그레이션 (Daum → Kakao)

> 카카오가 우편번호 서비스를 Daum 브랜드에서 Kakao 브랜드로 이전하면서,  
> 기존 도메인 및 JavaScript API 네임스페이스를 교체해야 함.  
> **2026년 4~5월** 기존 도메인 서비스 종료 예정.
...
| `-r` | 디렉토리를 재귀적으로 탐색 |
| `-n` | 검색 결과에 줄 번호 표시 |
| `-e "패턴"` | 검색할 패턴 지정. 여러 개 사용 시 각각 `-e` 붙임 |
| `--include="*.확장자"` | 해당 확장자 파일만 검색 대상으로 한정 |
| `/your/project/path` | 탐색할 루트 디렉토리 경로 |
| `2>/dev/null` | Permission denied 등 에러 메시지를 화면에 출력하지 않고 버림 |
| `> 파일명.txt` | 검색 결과를 파일로 저장 (붙이면 저장, 없으면 터미널 출력) |
...
grep -rn \
  ...
  --include="*.php" \
  --include="*.jsp" \
  /your/project/path 2>/dev/null > daum_to_kakao_targets.txt
```

### ✅ 교체 내용 요약
| 구분 | ❌ 기존 | ✅ 변경 |
|---|---|---|
| CDN 도메인 | `t1.daumcdn.net` | `t1.kakaocdn.net` |
| 서비스 팝업 | `postcode.map.daum.net` | `postcode.map.kakao.com` |
| JS 네임스페이스 | `new daum.Postcode()` | `new kakao.Postcode()` |
| **즉시 중단** | `dmaps.daum.net` | 신규 Kakao 도메인으로 전환 |
| **즉시 중단** | `ssl.daumcdn.net` | 신규 Kakao 도메인으로 전환 |
